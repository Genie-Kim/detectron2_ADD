...
from detectron2.modeling.roi_heads import RROIHeads
from detectron2.modeling.roi_heads.rotated_fast_rcnn import RotatedFastRCNNOutputs, fast_rcnn_inference_rotated
from torch import nn
from torch.autograd.function import Function

from detectron2.layers import ShapeSpec, batched_nms_rotated
from detectron2.structures import Boxes, Instances, pairwise_iou, RotatedBoxes, pairwise_iou_rotated
...
class CascadeRROIHeads(StandardROIHeads):
    ...
    def _init_box_head(self, cfg, input_shape):
        ...
        for k in range(self.num_cascade_stages):
            ...
            self.box_predictor.append(
                FastRCNNOutputLayers(
                    box_head.output_size, self.num_classes, cls_agnostic_bbox_reg=True,box_dim=5
                )
            )
            self.box2box_transform.append(Box2BoxTransformRotated(weights=cascade_bbox_reg_weights[k]))
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            if len(targets_per_image) > 0:
                ...
                gt_boxes = RotatedBoxes(
                    targets_per_image.gt_boxes.tensor.new_zeros((len(proposals_per_image), 5))
                )
            proposals_per_image.gt_classes = gt_classes
            proposals_per_image.gt_boxes = gt_boxes
